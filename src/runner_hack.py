...
class ImpulseRunner:
    ...
    def send_msg(self, msg):
        t_send_msg = now()
        print(f'----- | el mensaje en el send  {msg}')
        if not self._client:
            raise Exception('ImpulseRunner is not initialized')

        self._ix = self._ix + 1
        ix = self._ix
        print(f'----- | el self.ix {self._ix}')

        msg["id"] = ix
        self._client.send(json.dumps(msg).encode('utf-8'))
        print(f'----- | el mensaje en el send.client {self._client}')
        t_sent_msg = now()
        ...
        print(f'----- | la data recv {data_decode}')
...
def main(argv):
    ...
    print(f'estos son los features : {features}')
